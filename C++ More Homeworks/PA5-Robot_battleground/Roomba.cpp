...
Roomba::Roomba() : Robot(3,3,10,"defaultroomba") {

}
Roomba::Roomba(World *wrld, int x,int y,string name) : Robot(wrld,x,y,3,3,10,name) {
...
int Roomba::getIntType() {
    return ROOMBA;
}
int Roomba::getDamage() {
    int damage;
    damage = Robot::getDamage(); // Get basic damage from Robot class
    damage += (rand() % strength) + 1; // Add second damage points to damage 
    cout << "Increased " << "damage of " << name << " with second damage. New damage is " << damage << endl;
    return damage;
}

int Roomba::move(int& a,int& b) {
