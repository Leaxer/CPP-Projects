...
Optimusprime::Optimusprime() : Humanic(1,100,100,"defaultOptimusprime") {

}
Optimusprime::Optimusprime(World* wrld, int x,int y,string name) : Humanic(wrld,x,y,1,100,100,name) {
...
int Optimusprime::getIntType() {
    return OPTIMUS;
}
int Optimusprime::getDamage() {
    int damage;
    damage = Humanic::getDamage(); // Call and get damage points from Humanic class function
    int random = rand() % 100; // Random number between 0-100
    if(random < 15) { // If number is less than 15, means %15 chance
        damage *= 2; // Multiply by two the current damage
        cout << getType() << " damage doubled with %15 chance." << "New damage is " << damage << " points!" << endl;
    }
    return damage;
}

int Optimusprime::move(int& a,int& b) {
